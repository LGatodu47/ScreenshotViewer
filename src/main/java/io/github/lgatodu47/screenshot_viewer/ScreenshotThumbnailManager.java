...
public class ScreenshotThumbnailManager implements ScreenshotViewerConfigListener {
    ...
    public ScreenshotThumbnailManager(ScreenshotViewer mod, ScreenshotViewerConfig config) {
        this.config = config;
        this.executor = Util.backgroundExecutor();
        mod.registerConfigListener(this);
    }

    @Override
    public void onConfigReloaded() {
        File screenshotFolder = new File(config.screenshotsFolder.get());
        File thumbnailFolder = new File(config.thumbnailFolder.get());
        CompressionRatio ratio = config.compressionRatio.get();
        if(screenshotFolder.equals(this.screenshotFolder) && thumbnailFolder == this.thumbnailFolder && ratio == this.ratio) {
            return;
        }
        if(screenshotFolder.equals(thumbnailFolder)) {
            ...
            config.thumbnailFolder.set(thumbnailFolder.getAbsolutePath());
        }
        boolean regenerate = ratio != this.ratio || thumbnailFolder != this.thumbnailFolder;
